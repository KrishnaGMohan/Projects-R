...
for (row in 1:nrow(corr)) {
    for (col in 1:ncol(corr)) {
        print(paste(colnames(corr)[row], "and", rownames(corr)[col],
...
sapply(stock_return, FUN = sharpe, simplify = FALSE, USE.NAMES = FALSE) # same as lapply
...
vapply(market_crash2, FUN = class, FUN.VALUE = character(1)) # fail
